...
module UniAppModuleConfig
  ...
  INFO_PRIVACIES = {
    ...
    'Geolocation-Tencent' => %w[NSLocationUsageDescription NSLocationWhenInUseUsageDescription NSLocationAlwaysUsageDescription NSLocationAlwaysAndWhenInUseUsageDescription],
  ...
  def apply(enabled_subspecs, options = {})
    ...
    info_plist = options.fetch(:info_plist, File.join(project_dir, 'sdk_uniapp_iosDemo', 'sdk_uniapp_iosDemo-Info.plist'))
  ...
  def apply_map_values(plist, enabled, values, strict)
    ...
    if enabled.include?('Geolocation-Tencent')
      api_key = required_value(values.fetch(:map_tencent, {}), :api_key, 'map_tencent.api_key', strict)
      changed ||= set_nested_value(plist, ['TencentLBSAPIKey'], api_key) if present?(api_key)
      changed ||= set_nested_value(
        plist,
        %w[NSLocationTemporaryUsageDescriptionDictionary PurposeKey],
        'Allow temporary access to precise location for location services.'
      )
    end
    changed
  end
  ...
  def warn_missing(label)
    warn "[UniAppModuleConfig] Missing config: #{label}. Please edit sdk_uniapp_iosDemo/uniapp_config.rb."
  end
