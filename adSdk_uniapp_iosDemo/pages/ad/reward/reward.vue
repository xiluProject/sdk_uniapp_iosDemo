<template>
	<view class="page">
		<view class="card">
			<view class="card-title">激励视频广告（加载成功自动播放）</view>
			<view class="form-row">
				<text class="label">广告位Id</text>
				<input class="input" v-model="posId" placeholder="请输入激励视频广告位 Id" />
			</view>
			<view class="btn-row">
...
export default {
	...
	onLoad(query) {
		...
		uni.setNavigationBarTitle({ title: decodeURIComponent(query.title || '激励视频广告') });
	},
	onUnload() {
		// #ifdef APP-PLUS
		xiluAd.destroyAd('rewardVideo');
		// #endif
	},
	methods: {
		log(msg) {
