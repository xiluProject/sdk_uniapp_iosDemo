<template>
	<view class="page">
		<view class="card">
			<view class="card-title">信息流自渲染广告</view>
			<view class="form-row">
				<text class="label">广告位Id</text>
				<input class="input" v-model="posId" placeholder="请输入信息流自渲染广告位 Id" />
			</view>
			<view class="btn-row">
		...
		<!-- 原生信息流广告悬浮层会精准覆盖到本占位区，并一直铺到屏幕底部 -->
		<view class="native-placeholder" id="nativePlaceholder">
			<text class="placeholder-text">信息流自渲染广告展示区域（原生层可上下滑动）</text>
		</view>
	</view>
</template>
...
export default {
	...
	onLoad(query) {
		...
		uni.setNavigationBarTitle({ title: decodeURIComponent(query.title || '信息流自渲染广告') });
	},
	onUnload() {
		// #ifdef APP-PLUS
