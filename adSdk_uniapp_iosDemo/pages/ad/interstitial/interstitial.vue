<template>
	<view class="page">
		<view class="card">
			<view class="card-title">插屏广告（加载成功自动弹出）</view>
			<view class="form-row">
				<text class="label">广告位Id</text>
				<input class="input" v-model="posId" placeholder="请输入插屏广告位 Id" />
			</view>
			<view class="btn-row">
...
export default {
	...
	onLoad(query) {
		...
		uni.setNavigationBarTitle({ title: decodeURIComponent(query.title || '插屏广告') });
	},
	onUnload() {
		// #ifdef APP-PLUS
		xiluAd.destroyAd('interstitial');
		// #endif
	},
	methods: {
		log(msg) {
